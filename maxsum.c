...
{
  int i;
  for (i = 0; i < n; i++) x[i] = 1 - 2 * ((float)rand() / RAND_MAX);
}
...
float alg2b() {
  ...
  cumarr[-1] = 0;
  for (i = 0; i < n; i++) cumarr[i] = cumarr[i - 1] + x[i];
  for (i = 0; i < n; i++) {
    for (j = i; j < n; j++) {
...
float maxfun(float a, float b) { return a > b ? a : b; }
...
float alg3() { return recmax(0, n - 1); }
