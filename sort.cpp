...
void isort1() {
  int i, j;
  for (i = 1; i < n; i++)
    for (j = i; j > 0 && x[j - 1] > x[j]; j--) swap(j - 1, j);
}
...
void isort3() {
  ...
  for (i = 1; i < n; i++) {
    t = x[i];
    for (j = i; j > 0 && x[j - 1] > t; j--) x[j] = x[j - 1];
    x[j] = t;
  }
}
...
void timedriver() {
  ...
  while (scanf("%d %d %d", &algnum, &n, &mod) != EOF) {
    if (mod <= 0) mod = 10 * n;
    for (i = 0; i < n; i++) x[i] = randint(0, mod - 1);
    k = n / 2;
    start = clock();
    switch (algnum) {
      case 11:
        qsort(x, n, sizeof(int),
    ...
    if (algnum == 41) { /* Test selection */
      for (i = 0; i < k; i++)
        if (x[i] > x[k]) printf("  SELECT BUG i=%d\n", i);
      for (i = k + 1; i < n; i++)
        if (x[i] < x[k]) printf("  SELECT BUG i=%d\n", i);
    } else { /* Test sort */
      for (i = 0; i < n - 1; i++)
        if (x[i] > x[i + 1]) printf("  SORT BUG i=%d\n", i);
    }
    printf("%d\t%d\t%d\t%d\t%g\n", algnum, n, mod, clicks,
